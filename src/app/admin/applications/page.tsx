      ...
      {apps.length === 0 ? (
        ...
        <div className="space-y-4">
          {apps.map((app) => {
            ...
            return (
              <div key={app.id} className="glass-card rounded-2xl overflow-hidden transition-all">
                {/* Summary row */}
                <button
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      {app.plan_types?.map((pt) => (
                        <span key={pt} className={`px-2 py-0.5 rounded-md text-[10px] font-bold border ${PLAN_COLOR[pt] || "text-muted border-white/10"}`}>
                          {PLAN_LABEL[pt] || pt}
                        </span>
                      ))}
                    </div>
                ...
                {/* Expanded detail */}
                {isExpanded && (
                  <div className="border-t border-white/10 px-6 py-6 bg-white/5 space-y-6">
                    {/* Contact */}
                    ...
                    <div>
                      ...
                      <p className="text-white/80 text-sm leading-relaxed bg-white/5 rounded-xl p-4 border border-white/10">{app.bio || "No bio provided."}</p>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
                          {Object.entries(grouped).map(([day, times]) => (
                            <div key={day} className="bg-white/5 border border-white/10 rounded-lg p-3">
                              <p className="text-white text-xs font-semibold mb-1.5">{DAY_SHORT[day] || day}</p>
                              <div className="flex flex-wrap gap-1">
                                {times.map((t) => (
                                  <span key={t} className="px-2 py-0.5 bg-pink/10 text-pink text-[10px] font-medium rounded">{t}</span>
                                ))}
                              </div>
                            </div>
                          ))}
                    ...
                    <div className="flex gap-3 pt-2">
                      <button
                        ...
                        className="flex-1 sm:flex-none px-8 py-3 btn-gradient text-white rounded-xl text-sm font-bold hover:opacity-90 transition-all disabled:opacity-50"
                      >
