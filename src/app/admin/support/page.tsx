...
const STATUS_STYLE: Record<string, string> = {
  open: "bg-gold/10 text-gold", replied: "bg-pink/10 text-pink", closed: "bg-white/5 text-muted",
};
        ...
        {(["all", "open", "replied", "closed"] as const).map((f) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all capitalize ${
              filter === f ? "bg-pink text-white" : "text-gray-400 hover:text-white"
            }`}
          >
            {f} ({counts[f]})
          </button>
              ...
              <div className="p-6">
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {ticket.admin_reply && (
                      <div className="mt-3 p-3 bg-pink/5 border border-pink/10 rounded-lg">
                        <p className="text-pink text-[10px] font-bold uppercase tracking-wider mb-1">Your reply</p>
                    ...
                    {ticket.status !== "closed" && (
                      <div className="mt-3 flex gap-2">
                        <input
                          ...
                          className="flex-1 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white text-sm placeholder:text-muted/50 focus:outline-none focus:border-pink/40"
                        />
                        <button
                          onClick={() => handleReply(ticket.id)}
                          disabled={replyingId === ticket.id || !replyText[ticket.id]?.trim()}
                          className="px-4 py-2 btn-gradient text-white rounded-lg text-xs font-bold hover:opacity-90 transition-all disabled:opacity-50"
                        >
