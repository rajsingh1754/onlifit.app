    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <input
          ...
          className="px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white text-sm placeholder:text-muted/50 focus:outline-none focus:border-pink/40 sm:w-72"
        />
          ...
          {["all", "offline", "virtual", "elite"].map((p) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all capitalize ${
                planFilter === p ? "bg-pink text-white" : "text-gray-400 hover:text-white"
              }`}
            >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4">
          {filtered.map((t) => (
            <div key={t.id} className="glass-card rounded-2xl p-5 hover:border-white/20 transition-all">
              <div className="flex flex-col sm:flex-row gap-5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <Link href={`/admin/trainers/${t.id}`} className="text-white font-semibold text-lg hover:text-pink transition-colors">
                      {t.profile.full_name}
                    </Link>
                    {t.plan_types?.map((pt) => (
                    ...
                    {t.is_verified && <span className="text-pink text-xs">✓ Verified</span>}
                  </div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="px-3 py-2 bg-white/5 border border-white/10 rounded-lg">
                      ...
                      <p className="text-pink text-sm font-semibold">₹{t.monthlyEarnings.toLocaleString("en-IN")}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex sm:flex-col gap-2 flex-shrink-0">
                  <Link
                    ...
                    className="px-5 py-2.5 bg-pink/10 text-pink border border-pink/20 rounded-xl text-xs font-bold hover:bg-pink/20 transition-all text-center"
                  >
                    View Profile
                  </Link>
                  <button
